...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    // Developer command to reset limits
    if (message.trim() === '/reset-123') {
      if (isAuth && userId) {
        await User.findByIdAndUpdate(userId, { chat_count: 0 });
      }
      const clientIp = getClientIp(req);
      await Visitor.findOneAndUpdate({ ip: clientIp }, { chat_count: 0 }, { upsert: true, setDefaultsOnInsert: true });
      return res.json({ response: '⚙️ Developer command: Chat limits have been reset.' });
    }
    ...
    if (isAuth && userId) {
      ...
      if (user.chat_count >= 5) {
        return res.status(403).json({ error: 'LIMIT_REACHED', message: 'You have reached the maximum of 5 extra messages.' });
      }
      // Increment user chat count
      user.chat_count += 1;
      await user.save();
    } else {
      // Check visitor limit by IP
      const clientIp = getClientIp(req);
      let visitor = await Visitor.findOne({ ip: clientIp });
      if (!visitor) {
        visitor = new Visitor({ ip: clientIp, chat_count: 0 });
      }
      if (visitor.chat_count >= 3) {
        return res.status(403).json({ error: 'LIMIT_REACHED_UNAUTH', message: 'You have reached your 3 free messages. Please log in to get 5 extra messages.' });
      }
      // Increment visitor chat count
      visitor.chat_count += 1;
      await visitor.save();
    }
    ...
    if (!aiRes.ok) {
      ...
      throw new Error(`AI backend returned ${aiRes.status}: ${errText}`);
    }
